    ...
    var loader = function(p, div, src_domain) {
        ...
        return {
                    ...
                    loadJS = function(script, fn) {
                        ...
                        if (hasScript(d)) {
                            ...
                            $.getScript(d)
                                ...
                                .fail(function(jqxhhr, settings, exception) {
                                    console.log('ERROR LOADING: ', d);
                                    console.log(jqxhhr, settings, exception);
                                    callback(fn);
                                });
                        }
                    },
            ...
            CSS: function(success) {
                ...
                // discard non-truthy values
                cssArr = cssArr.filter(function(n) { return !!n; });
                deferLoad({
                    arr: cssArr,
                    success: success,
                    fn: function(item, fn) {
                        var d = (item.match(/http/)) ? item : params.src_domain + item;
                        $.getCSS(d, function() {
                            $('link[rel=stylesheet][href^="'+d+'"]').attr('title','page');
                            callback(fn);
                        });
                    }
                });
