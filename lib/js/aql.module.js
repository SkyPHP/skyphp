...
var Module = (function() {
	...
	Module.prototype = {
		...
		moduleHandlers: function(options) {
			...
			return this.merge(options, {
				success: function(json, $div) {
					...
					var scope = this, //  this will be set in aq; callback.
					...
					sky.call(continuation, scope, json, $div);
				...
				error: function(json, $div, errors) {
					...
					that.errors = errors;
					sky.call(continuation, scope, json, $div, errors);

				},
				remove: function(json, $div) {
					
					var scope = this,
						continuation = options.remove || that.handlers.remove;
					sky.call(continuation, scope, json, $div);
		...
		remove: function(options) {
			...
			options = this.moduleHandlers(options || {});
			options.success = options.remove;

			var data = {};
			data[this.model + '_ide'] = this.ide;
			data['_token'] = this.token;

			aql.remove(this.model, this.data, options);
