...
**/
	public function add_table_name($table_name, $field) {
		...
		$f = '';
		foreach ($rf as $r) {
			if (strpos($r,'.') === false && !in_array(trim($r), self::comparisons())) {
...
**/
	...
	public static function comparisons() {
		return array('DISTINCT', 'distinct', 'SELECT', 'select', 'WHERE', 'where', 'FROM', 'from', 'CASE', 'case', 'WHEN', 'when', 'THEN', 'then', 'ELSE', 'else', 'upper', 'lower', 'UPPER', 'LOWER', '*', 'and','or','like','like','AND','OR','LIKE','ILIKE','IS','is','null','in','IN','not','NOT','NULL','false','FALSE','now()','NOW()','asc','ASC','desc','DESC', 'interval', 'INTERVAL', '-', '+', '=', 'true', 'TRUE');
	}
