...
**/
	public function profile($param1, $param2, $param3 = false) {
		...
		if ($aql) {
			...
			if (!is_numeric($id)) die('AQL Error: invalid identifer. '.$param2.' for model '.$param1.' '.self::error_on());
		//	print_pre($aql);
			$clause = array(
				$model => array(
					'where' => array(
			...
			$rs = self::select($aql, $clause, $param3);
			return $rs[0];
		} else {
			return false;
		}
...
**/
	public function select($aql, $clause_array = null, $object = false) {
		global $db;
		...
		return self::sql_result($returned, $object);
	}
/**
...
**/
	...
	public function sql_result($arr, $object = false) {
		global $db;
		$rs = array();
